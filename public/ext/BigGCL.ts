...
;class JSBI_gcl extends Array {
  ...
  constructor(length: number, private sign: boolean) {
    ...
    Object.setPrototypeOf(this, JSBI_gcl.prototype);
    if (length > JSBI_gcl.__kMaxLength) {
  ...
  static BigInt(arg: number|string|boolean|object) :JSBI_gcl {
    if (typeof arg === 'number') {
      ...
      if (JSBI_gcl.__isOneDigitInt(arg)) {
        if (arg < 0) {
          return JSBI_gcl.__oneDigit(-arg, true);
        }
        return JSBI_gcl.__oneDigit(arg, false);
      }
      if (!Number.isFinite(arg) || Math.floor(arg) !== arg) {
        throw new RangeError('The number ' + arg + ' cannot be converted to ' +
                             'BigInt because it is not an integer');
      }
      return JSBI_gcl.__fromDouble(arg);
    } else if (typeof arg === 'string') {
      const result = JSBI_gcl.__fromString(arg);
      if (result === null) {
      ...
      if (arg === true) {
        return JSBI_gcl.__oneDigit(1, false);
      }
      return JSBI_gcl.__zero();
    } else if (typeof arg === 'object') {
      if (arg.constructor === JSBI_gcl) return arg;
      const primitive = JSBI_gcl.__toPrimitive(arg);
      return JSBI_gcl.BigInt(primitive);
    }
  ...
  override toString(radix: number = 10): string {
    ...
    if ((radix & (radix - 1)) === 0) {
      return JSBI_gcl.__toStringBasePowerOfTwo(this, radix);
    }
    return JSBI_gcl.__toStringGeneric(this, radix, false);
  }
  ...
  // TODO: add more tests
  static toNumber(x :JSBI_gcl): number {
    ...
    const msdLeadingZeros = JSBI_gcl.__clz30(xMsd);
    ...
    const rounding = JSBI_gcl.__decideRounding(x, mantissaLowBitsUnset,
    ...
    JSBI_gcl.__kBitConversionInts[1] = signBit | exponent | mantissaHigh;
    JSBI_gcl.__kBitConversionInts[0] = mantissaLow;
    return JSBI_gcl.__kBitConversionDouble[0];
  }

  // Operations.

  static unaryMinus(x :JSBI_gcl) :JSBI_gcl {
  ...
  static bitwiseNot(x :JSBI_gcl) :JSBI_gcl {
    if (x.sign) {
      // ~(-x) == ~(~(x-1)) == x-1
      return JSBI_gcl.__absoluteSubOne(x).__trim();
    }
    // ~x == -x-1 == -(x+1)
    return JSBI_gcl.__absoluteAddOne(x, true);
  }

  static exponentiate(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (y.sign) {
    ...
    if (expValue >= JSBI_gcl.__kMaxLengthBits) {
    ...
    if (x.length === 1 && x.__digit(0) === 2) {
      ...
      const result = new JSBI_gcl(neededDigits, sign);
    ...
    for (; expValue !== 0; expValue >>= 1) {
      runningSquare = JSBI_gcl.multiply(runningSquare, runningSquare);
      if ((expValue & 1) !== 0) {
        if (result === null) {
          result = runningSquare;
        } else {
          result = JSBI_gcl.multiply(result, runningSquare);
        }
      }
    }
    // TODO see if there's a way for tsc to infer this will always happen?
    return result as JSBI_gcl;
  }

  static multiply(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (x.length === 0) return x;
    ...
    const result = new JSBI_gcl(resultLength, x.sign !== y.sign);
    result.__initializeDigits();
    for (let i = 0; i < x.length; i++) {
      JSBI_gcl.__multiplyAccumulate(y, x.__digit(i), result, i);
    }
    return result.__trim();
  }

  static divide(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (y.length === 0) throw new RangeError('Division by zero');
    if (JSBI_gcl.__absoluteCompare(x, y) < 0) return JSBI_gcl.__zero();
    const resultSign = x.sign !== y.sign;
    const divisor = y.__unsignedDigit(0);
    let quotient;
    if (y.length === 1 && divisor <= 0x7FFF) {
      if (divisor === 1) {
        return resultSign === x.sign ? x  :JSBI_gcl.unaryMinus(x);
      }
      quotient = JSBI_gcl.__absoluteDivSmall(x, divisor, null);
    } else {
      quotient = JSBI_gcl.__absoluteDivLarge(x, y, true, false);
    }
    quotient.sign = resultSign;
    return quotient.__trim();
  }

  static remainder(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (y.length === 0) throw new RangeError('Division by zero');
    if (JSBI_gcl.__absoluteCompare(x, y) < 0) return x;
    const divisor = y.__unsignedDigit(0);
    if (y.length === 1 && divisor <= 0x7FFF) {
      if (divisor === 1) return JSBI_gcl.__zero();
      const remainderDigit = JSBI_gcl.__absoluteModSmall(x, divisor);
      if (remainderDigit === 0) return JSBI_gcl.__zero();
      return JSBI_gcl.__oneDigit(remainderDigit, x.sign);
    }
    const remainder = JSBI_gcl.__absoluteDivLarge(x, y, false, true);
    remainder.sign = x.sign;
    return remainder.__trim();
  }

  static add(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    const sign = x.sign;
    if (sign === y.sign) {
      // x + y == x + y
      // -x + -y == -(x + y)
      return JSBI_gcl.__absoluteAdd(x, y, sign);
    }
    // x + -y == x - y == -(y - x)
    // -x + y == y - x == -(x - y)
    if (JSBI_gcl.__absoluteCompare(x, y) >= 0) {
      return JSBI_gcl.__absoluteSub(x, y, sign);
    }
    return JSBI_gcl.__absoluteSub(y, x, !sign);
  }

  static subtract(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    const sign = x.sign;
    if (sign !== y.sign) {
      // x - (-y) == x + y
      // (-x) - y == -(x + y)
  ...
  static leftShift(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (y.length === 0 || x.length === 0) return x;
    if (y.sign) return JSBI_gcl.__rightShiftByAbsolute(x, y);
    return JSBI_gcl.__leftShiftByAbsolute(x, y);
  }

  static signedRightShift(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (y.length === 0 || x.length === 0) return x;
    if (y.sign) return JSBI_gcl.__leftShiftByAbsolute(x, y);
    return JSBI_gcl.__rightShiftByAbsolute(x, y);
  }
  ...
  static lessThan(x :JSBI_gcl, y :JSBI_gcl): boolean {
    return JSBI_gcl.__compareToBigInt(x, y) < 0;
  }

  static lessThanOrEqual(x :JSBI_gcl, y :JSBI_gcl): boolean {
    return JSBI_gcl.__compareToBigInt(x, y) <= 0;
  }

  static greaterThan(x :JSBI_gcl, y :JSBI_gcl): boolean {
    return JSBI_gcl.__compareToBigInt(x, y) > 0;
  }

  static greaterThanOrEqual(x :JSBI_gcl, y :JSBI_gcl): boolean {
    return JSBI_gcl.__compareToBigInt(x, y) >= 0;
  }

  static equal(x :JSBI_gcl, y :JSBI_gcl): boolean {
  ...
  static notEqual(x :JSBI_gcl, y :JSBI_gcl): boolean {
    return !JSBI_gcl.equal(x, y);
  }

  static bitwiseAnd(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (!x.sign && !y.sign) {
      return JSBI_gcl.__absoluteAnd(x, y).__trim();
    } else if (x.sign && y.sign) {
      const resultLength = Math.max(x.length, y.length) + 1;
      // (-x) & (-y) == ~(x-1) & ~(y-1) == ~((x-1) | (y-1))
      // == -(((x-1) | (y-1)) + 1)
      let result = JSBI_gcl.__absoluteSubOne(x, resultLength);
      const y1 = JSBI_gcl.__absoluteSubOne(y);
      result = JSBI_gcl.__absoluteOr(result, y1, result);
      return JSBI_gcl.__absoluteAddOne(result, true, result).__trim();
    }
    ...
    // x & (-y) == x & ~(y-1) == x &~ (y-1)
    return JSBI_gcl.__absoluteAndNot(x, JSBI_gcl.__absoluteSubOne(y)).__trim();
  }

  static bitwiseXor(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    if (!x.sign && !y.sign) {
      return JSBI_gcl.__absoluteXor(x, y).__trim();
    } else if (x.sign && y.sign) {
      // (-x) ^ (-y) == ~(x-1) ^ ~(y-1) == (x-1) ^ (y-1)
      const resultLength = Math.max(x.length, y.length);
      const result = JSBI_gcl.__absoluteSubOne(x, resultLength);
      const y1 = JSBI_gcl.__absoluteSubOne(y);
      return JSBI_gcl.__absoluteXor(result, y1, result).__trim();
    }
    const resultLength = Math.max(x.length, y.length) + 1;
    // Assume that x is the positive BigInt.
    if (x.sign) {
    ...
    let result = JSBI_gcl.__absoluteSubOne(y, resultLength);
    result = JSBI_gcl.__absoluteXor(result, x, result);
    return JSBI_gcl.__absoluteAddOne(result, true, result).__trim();
  }

  static bitwiseOr(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    const resultLength = Math.max(x.length, y.length);
    if (!x.sign && !y.sign) {
      return JSBI_gcl.__absoluteOr(x, y).__trim();
    } else if (x.sign && y.sign) {
      // (-x) | (-y) == ~(x-1) | ~(y-1) == ~((x-1) & (y-1))
      // == -(((x-1) & (y-1)) + 1)
      let result = JSBI_gcl.__absoluteSubOne(x, resultLength);
      const y1 = JSBI_gcl.__absoluteSubOne(y);
      result = JSBI_gcl.__absoluteAnd(result, y1, result);
      return JSBI_gcl.__absoluteAddOne(result, true, result).__trim();
    }
    // Assume that x is the positive BigInt.
    if (x.sign) {
    ...
    let result = JSBI_gcl.__absoluteSubOne(y, resultLength);
    result = JSBI_gcl.__absoluteAndNot(result, x, result);
    return JSBI_gcl.__absoluteAddOne(result, true, result).__trim();
  }

  static asIntN(n: number, x :JSBI_gcl) :JSBI_gcl {
    if (x.length === 0) return x;
    n = Math.floor(n);
    if (n < 0) {
      throw new RangeError(
    ...
    if (n === 0) return JSBI_gcl.__zero();
    // If {x} has less than {n} bits, return it directly.
    if (n >= JSBI_gcl.__kMaxLengthBits) return x;
    ...
    if (!hasBit) return JSBI_gcl.__truncateToNBits(n, x);
    if (!x.sign) return JSBI_gcl.__truncateAndSubFromPowerOfTwo(n, x, true);
    if ((topDigit & (compareDigit - 1)) === 0) {
      for (let i = neededLength - 2; i >= 0; i--) {
        if (x.__digit(i) !== 0) {
          return JSBI_gcl.__truncateAndSubFromPowerOfTwo(n, x, false);
        }
      }
      if (x.length === neededLength && topDigit === compareDigit) return x;
      return JSBI_gcl.__truncateToNBits(n, x);
    }
    return JSBI_gcl.__truncateAndSubFromPowerOfTwo(n, x, false);
  }

  static asUintN(n: number, x :JSBI_gcl) :JSBI_gcl {
    if (x.length === 0) return x;
    n = Math.floor(n);
    if (n < 0) {
      throw new RangeError(
    ...
    if (x.sign) {
      if (n > JSBI_gcl.__kMaxLengthBits) {
        throw new RangeError('BigInt too big');
      }
      return JSBI_gcl.__truncateAndSubFromPowerOfTwo(n, x, false);
    }
  ...
  static ADD(x: any, y: any) {
    x = JSBI_gcl.__toPrimitive(x);
    y = JSBI_gcl.__toPrimitive(y);
    if (typeof x === 'string') {
    ...
    x = JSBI_gcl.__toNumeric(x);
    y = JSBI_gcl.__toNumeric(y);
    if (JSBI_gcl.__isBigInt(x) && JSBI_gcl.__isBigInt(y)) {
      return JSBI_gcl.add(x, y);
    }
  ...
  static LT(x: any, y: any): boolean {
    return JSBI_gcl.__compare(x, y, 0);
  }
  static LE(x: any, y: any): boolean {
    return JSBI_gcl.__compare(x, y, 1);
  }
  static GT(x: any, y: any): boolean {
    return JSBI_gcl.__compare(x, y, 2);
  }
  static GE(x: any, y: any): boolean {
    return JSBI_gcl.__compare(x, y, 3);
  }

  static EQ(x: any, y: any): boolean {
    while (true) {
      if (JSBI_gcl.__isBigInt(x)) {
        if (JSBI_gcl.__isBigInt(y)) return JSBI_gcl.equal(x, y);
        return JSBI_gcl.EQ(y, x);
      } else if (typeof x === 'number') {
        if (JSBI_gcl.__isBigInt(y)) return JSBI_gcl.__equalToNumber(y, x);
        if (typeof y !== 'object') return x == y;
        y = JSBI_gcl.__toPrimitive(y);
      } else if (typeof x === 'string') {
        if (JSBI_gcl.__isBigInt(y)) {
          x = JSBI_gcl.__fromString(x);
        ...
        if (JSBI_gcl.__isBigInt(y)) return false;
        if (typeof y !== 'object') return x == y;
        y = JSBI_gcl.__toPrimitive(y);
      } else if (typeof x === 'object') {
        if (typeof y === 'object' && y.constructor !== JSBI_gcl) return x == y;
        x = JSBI_gcl.__toPrimitive(x);
      } else {
        return x == y;
      }
  ...
  static DataViewGetBigInt64(
    ...
    return JSBI_gcl.asIntN(
        64, JSBI_gcl.DataViewGetBigUint64(dataview, byteOffset, littleEndian));
  }
  ...
  static DataViewGetBigUint64(
    ...
    const result = new JSBI_gcl(3, false);
  ...
  static DataViewSetBigInt64(
      dataview: DataView, byteOffset: number, value :JSBI_gcl,
      littleEndian: boolean = false) {
    JSBI_gcl.DataViewSetBigUint64(dataview, byteOffset, value, littleEndian);
  }

  static DataViewSetBigUint64(
      dataview: DataView, byteOffset: number, value :JSBI_gcl,
      littleEndian: boolean = false) {
    value = JSBI_gcl.asUintN(64, value);
    let high = 0;
    let low = 0;
    if (value.length > 0) {
      low = value.__digit(0);
  ...
  static __zero() :JSBI_gcl {
    return new JSBI_gcl(0, false);
  }

  static __oneDigit(value: number, sign: boolean) :JSBI_gcl {
    const result = new JSBI_gcl(1, sign);
    result.__setDigit(0, value);
    return result;
  }

  __copy() :JSBI_gcl {
    const result = new JSBI_gcl(this.length, this.sign);
  ...
  static __decideRounding(x :JSBI_gcl, mantissaBitsUnset: number,
  ...
  static __fromDouble(value: number) :JSBI_gcl {
    const sign = value < 0;
    JSBI_gcl.__kBitConversionDouble[0] = value;
    const rawExponent = (JSBI_gcl.__kBitConversionInts[1] >>> 20) & 0x7FF;
    const exponent = rawExponent - 0x3FF;
    const digits = ((exponent / 30) | 0) + 1;
    const result = new JSBI_gcl(digits, sign);
    const kHiddenBit = 0x00100000;
    let mantissaHigh = (JSBI_gcl.__kBitConversionInts[1] & 0xFFFFF) | kHiddenBit;
    let mantissaLow = JSBI_gcl.__kBitConversionInts[0];
  ...
  static __fromString(string: string, radix:number = 0) :JSBI_gcl|null {
    let sign = 0;
    let leadingZero = false;
    const length = string.length;
    let cursor = 0;
    if (cursor === length) return JSBI_gcl.__zero();
    let current = string.charCodeAt(cursor);
    // Skip whitespace.
    while (JSBI_gcl.__isWhitespace(current)) {
      if (++cursor === length) return JSBI_gcl.__zero();
    ...
    let bitsPerChar = JSBI_gcl.__kMaxBitsPerChar[radix];
    let roundup = JSBI_gcl.__kBitsPerCharTableMultiplier - 1;
    if (chars > (1 << 30) / bitsPerChar) return null;
    const bitsMin =
        (bitsPerChar * chars + roundup) >>> JSBI_gcl.__kBitsPerCharTableShift;
    const resultLength = ((bitsMin + 29) / 30) | 0;
    const result = new JSBI_gcl(resultLength, false);
    ...
    if ((radix & (radix - 1)) === 0) {
      // Power-of-two radix.
      bitsPerChar >>= JSBI_gcl.__kBitsPerCharTableShift;
      const parts = [];
      const partsBits = [];
      let done = false;
      do {
      ...
      JSBI_gcl.__fillFromParts(result, parts, partsBits);
    } else {
      result.__initializeDigits();
      let done = false;
      let charsSoFar = 0;
      do {
        ...
        roundup = JSBI_gcl.__kBitsPerCharTableMultiplier * 30 - 1;
        const digitsSoFar = (((bitsPerChar * charsSoFar + roundup) >>>
                             JSBI_gcl.__kBitsPerCharTableShift) / 30) | 0;
        result.__inplaceMultiplyAdd(multiplier, part, digitsSoFar);
      } while (!done);
    }

    if (cursor !== length) {
      if (!JSBI_gcl.__isWhitespace(current)) return null;
  ...
  static __fillFromParts(result :JSBI_gcl, parts: number[], partsBits: number[]):
      void {
    let digitIndex = 0;
    let digit = 0;
    let bitsInDigit = 0;
  ...
  static __toStringBasePowerOfTwo(x :JSBI_gcl, radix: number): string {
    ...
    const msdLeadingZeros = JSBI_gcl.__clz30(msd);
    ...
    for (let i = 0; i < length - 1; i++) {
      ...
      result[pos--] = JSBI_gcl.__kConversionChars[current];
      const consumedBits = bitsPerChar - availableBits;
      digit = newDigit >>> consumedBits;
      availableBits = 30 - consumedBits;
      while (availableBits >= bitsPerChar) {
        result[pos--] = JSBI_gcl.__kConversionChars[digit & charMask];
        digit >>>= bitsPerChar;
  ...
  static __toStringGeneric(x :JSBI_gcl, radix: number, isRecursiveCall: boolean):
      string {
    const length = x.length;
    if (length === 0) return '';
    if (length === 1) {
    ...
    const bitLength = length * 30 - JSBI_gcl.__clz30(x.__digit(length - 1));
    const maxBitsPerChar = JSBI_gcl.__kMaxBitsPerChar[radix];
    const minBitsPerChar = maxBitsPerChar - 1;
    let charsRequired = bitLength * JSBI_gcl.__kBitsPerCharTableMultiplier;
    ...
    const conqueror = JSBI_gcl.exponentiate(JSBI_gcl.__oneDigit(radix, false),
        JSBI_gcl.__oneDigit(secondHalfChars, false));
    let quotient;
    let secondHalf;
    const divisor = conqueror.__unsignedDigit(0);
    if (conqueror.length === 1 && divisor <= 0x7FFF) {
      quotient = new JSBI_gcl(x.length, false);
      ...
      const divisionResult = JSBI_gcl.__absoluteDivLarge(x, conqueror, true, true);
      quotient = divisionResult.quotient;
      const remainder = divisionResult.remainder.__trim();
      secondHalf = JSBI_gcl.__toStringGeneric(remainder, radix, true);
    }
    quotient.__trim();
    let firstHalf = JSBI_gcl.__toStringGeneric(quotient, radix, true);
  ...
  static __compareToBigInt(x :JSBI_gcl, y :JSBI_gcl): number {
    const xSign = x.sign;
    if (xSign !== y.sign) return JSBI_gcl.__unequalSign(xSign);
    const result = JSBI_gcl.__absoluteCompare(x, y);
    if (result > 0) return JSBI_gcl.__absoluteGreater(xSign);
    if (result < 0) return JSBI_gcl.__absoluteLess(xSign);
    return 0;
  }

  static __compareToNumber(x :JSBI_gcl, y: number): number {
    if (JSBI_gcl.__isOneDigitInt(y)) {
      const xSign = x.sign;
      const ySign = (y < 0);
      if (xSign !== ySign) return JSBI_gcl.__unequalSign(xSign);
      if (x.length === 0) {
        if (ySign) throw new Error('implementation bug');
        return y === 0 ? 0 : -1;
      }
      // Any multi-digit BigInt is bigger than an int32.
      if (x.length > 1) return JSBI_gcl.__absoluteGreater(xSign);
      const yAbs = Math.abs(y);
      const xDigit = x.__unsignedDigit(0);
      if (xDigit > yAbs) return JSBI_gcl.__absoluteGreater(xSign);
      if (xDigit < yAbs) return JSBI_gcl.__absoluteLess(xSign);
      return 0;
    }
    return JSBI_gcl.__compareToDouble(x, y);
  }

  static __compareToDouble(x :JSBI_gcl, y: number): number {
    if (y !== y) return y; // NaN.
    ...
    JSBI_gcl.__kBitConversionDouble[0] = y;
    const rawExponent = (JSBI_gcl.__kBitConversionInts[1] >>> 20) & 0x7FF;
    ...
    if (exponent < 0) {
      ...
      return JSBI_gcl.__absoluteGreater(xSign);
    }
    const xLength = x.length;
    ...
    if (xBitLength < yBitLength) return JSBI_gcl.__absoluteLess(xSign);
    if (xBitLength > yBitLength) return JSBI_gcl.__absoluteGreater(xSign);
    ...
    if (xMsd > compareMantissa) return JSBI_gcl.__absoluteGreater(xSign);
    if (xMsd < compareMantissa) return JSBI_gcl.__absoluteLess(xSign);
    ...
    for (let digitIndex = xLength - 2; digitIndex >= 0; digitIndex--) {
      ...
      if (digit > compareMantissa) return JSBI_gcl.__absoluteGreater(xSign);
      if (digit < compareMantissa) return JSBI_gcl.__absoluteLess(xSign);
    }
    // Integer parts are equal; check whether {y} has a fractional part.
    if (mantissaHigh !== 0 || mantissaLow !== 0) {
      if (remainingMantissaBits === 0) throw new Error('implementation bug');
      return JSBI_gcl.__absoluteLess(xSign);
    }
    return 0;
  }

  static __equalToNumber(x :JSBI_gcl, y: number) {
    if (JSBI_gcl.__isOneDigitInt(y)) {
    ...
    return JSBI_gcl.__compareToDouble(x, y) === 0;
  }
  ...
  static __compare(x: any, y: any, op: 0|1|2|3): boolean {
    ...
    if (JSBI_gcl.__isBigInt(x) && typeof y === 'string') {
      y = JSBI_gcl.__fromString(y);
      if (y === null) return false;
      return JSBI_gcl.__comparisonResultToBool(JSBI_gcl.__compareToBigInt(x, y), op);
    }
    if (typeof x === 'string' && JSBI_gcl.__isBigInt(y)) {
      x = JSBI_gcl.__fromString(x);
      if (x === null) return false;
      return JSBI_gcl.__comparisonResultToBool(JSBI_gcl.__compareToBigInt(x, y), op);
    }
    ...
    if (JSBI_gcl.__isBigInt(x)) {
      ...
      return JSBI_gcl.__comparisonResultToBool(JSBI_gcl.__compareToNumber(x, y), op);
    }
    if (typeof x !== 'number') throw new Error('implementation bug');
    if (JSBI_gcl.__isBigInt(y)) {
      // Note that "op ^ 2" reverses the op's direction.
      return JSBI_gcl.__comparisonResultToBool(JSBI_gcl.__compareToNumber(y, x),
          (op ^ 2) as 0|1|2|3);
    }
    if (typeof y !== 'number') throw new Error('implementation bug');
    switch (op) {
  ...
  __clzmsd(): number {
    return JSBI_gcl.__clz30(this.__digit(this.length - 1));
  }

  static __absoluteAdd(x :JSBI_gcl, y :JSBI_gcl, resultSign: boolean) :JSBI_gcl {
    if (x.length < y.length) return JSBI_gcl.__absoluteAdd(y, x, resultSign);
    if (x.length === 0) return x;
    if (y.length === 0) return x.sign === resultSign ? x  :JSBI_gcl.unaryMinus(x);
    let resultLength = x.length;
    if (x.__clzmsd() === 0 || (y.length === x.length && y.__clzmsd() === 0)) {
      resultLength++;
    }
    const result = new JSBI_gcl(resultLength, resultSign);
    let carry = 0;
    let i = 0;
    for (; i < y.length; i++) {
  ...
  static __absoluteSub(x :JSBI_gcl, y :JSBI_gcl, resultSign: boolean) :JSBI_gcl {
    if (x.length === 0) return x;
    if (y.length === 0) return x.sign === resultSign ? x  :JSBI_gcl.unaryMinus(x);
    const result = new JSBI_gcl(x.length, resultSign);
    let borrow = 0;
    let i = 0;
    for (; i < y.length; i++) {
  ...
  static __absoluteAddOne(x :JSBI_gcl, sign: boolean, result :JSBI_gcl|null = null) {
    const inputLength = x.length;
    if (result === null) {
      result = new JSBI_gcl(inputLength, sign);
    } else {
      result.sign = sign;
    }
    let carry = 1;
  ...
  static __absoluteSubOne(x :JSBI_gcl, resultLength?: number) {
    const length = x.length;
  ...
  static __absoluteAnd(x :JSBI_gcl, y :JSBI_gcl, result :JSBI_gcl|null = null) {
    ...
    if (result === null) {
      result = new JSBI_gcl(resultLength, false);
    } else {
  ...
  static __absoluteAndNot(x :JSBI_gcl, y :JSBI_gcl, result :JSBI_gcl|null = null) {
    const xLength = x.length;
  ...
  static __absoluteOr(x :JSBI_gcl, y :JSBI_gcl, result :JSBI_gcl|null = null) {
    let xLength = x.length;
    let yLength = y.length;
    let numPairs = yLength;
    if (xLength < yLength) {
  ...
  static __absoluteXor(x :JSBI_gcl, y :JSBI_gcl, result :JSBI_gcl|null = null) {
    let xLength = x.length;
    let yLength = y.length;
    let numPairs = yLength;
    if (xLength < yLength) {
  ...
  static __absoluteCompare(x :JSBI_gcl, y :JSBI_gcl) {
  ...
  static __multiplyAccumulate(multiplicand :JSBI_gcl, multiplier: number,
      accumulator :JSBI_gcl, accumulatorIndex: number): void {
    ...
    for (let i = 0; i < multiplicand.length; i++, accumulatorIndex++) {
      ...
      const m1High = m1 >>> 15;
      const rLow = JSBI_gcl.__imul(m1Low, m2Low);
      const rMid1 = JSBI_gcl.__imul(m1Low, m2High);
      const rMid2 = JSBI_gcl.__imul(m1High, m2Low);
      const rHigh = JSBI_gcl.__imul(m1High, m2High);
      acc += high + rLow + carry;
      carry = acc >>> 30;
      acc &= 0x3FFFFFFF;
  ...
  static __internalMultiplyAdd(source :JSBI_gcl, factor: number, summand: number,
      n: number, result :JSBI_gcl): void {
    let carry = summand;
    let high = 0;
    for (let i = 0; i < n; i++) {
      const digit = source.__digit(i);
      const rx = JSBI_gcl.__imul(digit & 0x7FFF, factor);
      const ry = JSBI_gcl.__imul(digit >>> 15, factor);
  ...
  __inplaceMultiplyAdd(multiplier: number, summand: number, length: number):
    ...
    for (let i = 0; i < length; i++) {
      ...
      const dLow = d & 0x7FFF;
      const dHigh = d >>> 15;
      const pLow = JSBI_gcl.__imul(dLow, mLow);
      const pMid1 = JSBI_gcl.__imul(dLow, mHigh);
      const pMid2 = JSBI_gcl.__imul(dHigh, mLow);
      const pHigh = JSBI_gcl.__imul(dHigh, mHigh);
  ...
  static __absoluteDivSmall(x :JSBI_gcl, divisor: number,
      quotient :JSBI_gcl|null = null) :JSBI_gcl {
    if (quotient === null) quotient = new JSBI_gcl(x.length, false);
    let remainder = 0;
  ...
  static __absoluteModSmall(x :JSBI_gcl, divisor: number): number {
    let remainder = 0;
    for (let i = x.length * 2 - 1; i >= 0; i--) {
  ...
  static __absoluteDivLarge(dividend :JSBI_gcl, divisor :JSBI_gcl, wantQuotient: false,
      wantRemainder: false): undefined;
  static __absoluteDivLarge(dividend :JSBI_gcl, divisor :JSBI_gcl, wantQuotient: true,
      wantRemainder: true): { quotient :JSBI_gcl; remainder :JSBI_gcl; };
  static __absoluteDivLarge(dividend :JSBI_gcl, divisor :JSBI_gcl,
      wantQuotient: boolean, wantRemainder: boolean) :JSBI_gcl;
  static __absoluteDivLarge(dividend :JSBI_gcl, divisor :JSBI_gcl,
      wantQuotient: boolean, wantRemainder: boolean):
      { quotient :JSBI_gcl; remainder :JSBI_gcl; }|JSBI_gcl|undefined {
    const n = divisor.__halfDigitLength();
    const n2 = divisor.length;
    const m = dividend.__halfDigitLength() - n;
    let q = null;
    if (wantQuotient) {
      q = new JSBI_gcl((m + 2) >>> 1, false);
      q.__initializeDigits();
    }
    const qhatv = new JSBI_gcl((n + 2) >>> 1, false);
    qhatv.__initializeDigits();
    // D1.
    const shift = JSBI_gcl.__clz15(divisor.__halfDigit(n - 1));
    if (shift > 0) {
      divisor = JSBI_gcl.__specialLeftShift(divisor, shift, 0); // add no digits
    }
    const u = JSBI_gcl.__specialLeftShift(dividend, shift, 1); // add one digit
    // D2.
    ...
    for (let j = m; j >= 0; j--) {
      ...
      if (ujn !== vn1) {
        ...
        while ((JSBI_gcl.__imul(qhat, vn2) >>> 0) > (((rhat << 16) | ujn2) >>> 0)) {
          qhat--;
          rhat += vn1;
      ...
      // D4.
      JSBI_gcl.__internalMultiplyAdd(divisor, qhat, 0, n2, qhatv);
      ...
      if (wantQuotient) {
        if (j & 1) {
          ...
          (q as JSBI_gcl).__setDigit(j >>> 1, halfDigitBuffer | qhat);
        }
      }
    }
    if (wantRemainder) {
      ...
      if (wantQuotient) {
        return {quotient: (q as JSBI_gcl), remainder: u};
      }
      return u;
    }
    if (wantQuotient) return (q as JSBI_gcl);
    // TODO find a way to make this statically unreachable?
    throw new Error('unreachable');
  }

  static __clz15(value: number): number {
    return JSBI_gcl.__clz30(value) - 15;
  }

  // TODO: work on full digits, like __inplaceSub?
  __inplaceAdd(summand :JSBI_gcl, startIndex: number, halfDigits: number): number {
    let carry = 0;
  ...
  __inplaceSub(subtrahend :JSBI_gcl, startIndex: number, halfDigits: number):
      number {
  ...
  static __specialLeftShift(x :JSBI_gcl, shift: number, addDigit: 0|1) :JSBI_gcl {
    const n = x.length;
  ...
  static __leftShiftByAbsolute(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    const shift = JSBI_gcl.__toShiftAmount(y);
    ...
    const result = new JSBI_gcl(resultLength, x.sign);
    if (bitsShift === 0) {
      let i = 0;
  ...
  static __rightShiftByAbsolute(x :JSBI_gcl, y :JSBI_gcl) :JSBI_gcl {
    const length = x.length;
    const sign = x.sign;
    const shift = JSBI_gcl.__toShiftAmount(y);
    if (shift < 0) return JSBI_gcl.__rightShiftByMaximum(sign);
    const digitShift = (shift / 30) | 0;
    const bitsShift = shift % 30;
    let resultLength = length - digitShift;
    if (resultLength <= 0) return JSBI_gcl.__rightShiftByMaximum(sign);
    ...
    let result = new JSBI_gcl(resultLength, sign);
    if (bitsShift === 0) {
    ...
    if (mustRoundDown) {
      ...
      result = JSBI_gcl.__absoluteAddOne(result, true, result);
    }
    return result.__trim();
  }

  static __rightShiftByMaximum(sign: boolean) :JSBI_gcl {
    if (sign) {
      return JSBI_gcl.__oneDigit(1, true);
    }
    return JSBI_gcl.__zero();
  }

  static __toShiftAmount(x :JSBI_gcl): number {
    if (x.length > 1) return -1;
    const value = x.__unsignedDigit(0);
    if (value > JSBI_gcl.__kMaxLengthBits) return -1;
    return value;
  }

  static __toPrimitive(obj: any, hint='default'): any {
    if (typeof obj !== 'object') return obj;
    if (obj.constructor === JSBI_gcl) return obj;
  ...
  static __toNumeric(value: unknown): number|JSBI_gcl {
    if (JSBI_gcl.__isBigInt(value)) return value;
    return +(value as any);
  }

  static __isBigInt(value: unknown): value is JSBI_gcl {
    return typeof value === 'object' && value !== null &&
           value.constructor === JSBI_gcl;
  }

  static __truncateToNBits(n: number, x :JSBI_gcl) :JSBI_gcl {
    const neededDigits = ((n + 29) / 30) | 0;
    const result = new JSBI_gcl(neededDigits, x.sign);
  ...
  static __truncateAndSubFromPowerOfTwo(n: number, x :JSBI_gcl,
      resultSign: boolean) :JSBI_gcl {
    const neededDigits = ((n + 29) / 30) | 0;
    const result = new JSBI_gcl(neededDigits, resultSign);
    let i = 0;
    const last = neededDigits - 1;
    let borrow = 0;
  ...
  static __kMaxLengthBits = JSBI_gcl.__kMaxLength << 5;
  ...
  static __kBitsPerCharTableMultiplier = 1 << JSBI_gcl.__kBitsPerCharTableShift;
  static __kConversionChars = '0123456789abcdefghijklmnopqrstuvwxyz'.split('');
  static __kBitConversionBuffer = new ArrayBuffer(8);
  static __kBitConversionDouble = new Float64Array(JSBI_gcl.__kBitConversionBuffer);
  static __kBitConversionInts = new Int32Array(JSBI_gcl.__kBitConversionBuffer);
