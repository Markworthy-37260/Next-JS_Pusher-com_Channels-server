...
class JSBI_gcl extends Array {
    /* private /**/ constructor(length, sign) {
        super(length);
        this.sign = sign;
    ...
    static toNumber(x) {
        ...
        const rounding = JSBI_gcl.__decideRounding(x, mantissaLowBitsUnset, digitIndex, currentDigit);
    ...
    static asIntN(n, x) {
        ...
        if (!x.sign)
            return JSBI_gcl.__truncateAndSubFromPowerOfTwo(n, x, true);
    ...
    static EQ(x, y) {
        while (true) {
            if (JSBI_gcl.__isBigInt(x)) {
                ...
                if (JSBI_gcl.__isBigInt(y))
                    return JSBI_gcl.__equalToNumber(y, x);
                if (typeof y !== 'object')
                    return x == y;
                y = JSBI_gcl.__toPrimitive(y);
            }
                ...
                if (typeof y === 'object' && y.constructor !== JSBI_gcl)
                    return x == y;
                x = JSBI_gcl.__toPrimitive(x);
            }
            else {
    ...
    static DataViewGetBigInt64(dataview, byteOffset, littleEndian = false) {
        return JSBI_gcl.asIntN(64, JSBI_gcl.DataViewGetBigUint64(dataview, byteOffset, littleEndian));
    }
    ...
    static __fromString(string, radix = 0) {
        ...
        if (chars > (1 << 30) / bitsPerChar)
            return null;
        const bitsMin = (bitsPerChar * chars + roundup) >>> JSBI_gcl.__kBitsPerCharTableShift;
        ...
        if (cursor !== length) {
            if (!JSBI_gcl.__isWhitespace(current))
                return null;
    ...
    static __toStringGeneric(x, radix, isRecursiveCall) {
        ...
        const conqueror = JSBI_gcl.exponentiate(JSBI_gcl.__oneDigit(radix, false), JSBI_gcl.__oneDigit(secondHalfChars, false));
        let quotient;
        let secondHalf;
    ...
    static __compareToBigInt(x, y) {
        const xSign = x.sign;
        if (xSign !== y.sign)
            return JSBI_gcl.__unequalSign(xSign);
    ...
    static __compare(x, y, op) {
        ...
        if (JSBI_gcl.__isBigInt(y)) {
            ...
            return JSBI_gcl.__comparisonResultToBool(JSBI_gcl.__compareToNumber(y, x), (op ^ 2));
        }
        if (typeof y !== 'number')
            throw new Error('implementation bug');
        switch (op) {
    ...
    static __absoluteAdd(x, y, resultSign) {
        if (x.length < y.length)
            return JSBI_gcl.__absoluteAdd(y, x, resultSign);
        if (x.length === 0)
            return x;
        if (y.length === 0)
            return x.sign === resultSign ? x : JSBI_gcl.unaryMinus(x);
    ...
    static __rightShiftByAbsolute(x, y) {
        ...
        const shift = JSBI_gcl.__toShiftAmount(y);
        if (shift < 0)
            return JSBI_gcl.__rightShiftByMaximum(sign);
        const digitShift = (shift / 30) | 0;
        const bitsShift = shift % 30;
    ...
    static __toShiftAmount(x) {
        if (x.length > 1)
            return -1;
        const value = x.__unsignedDigit(0);
        if (value > JSBI_gcl.__kMaxLengthBits)
            return -1;
        return value;
    }
...
JSBI_gcl.__kMaxLength = 1 << 25;
JSBI_gcl.__kMaxLengthBits = JSBI_gcl.__kMaxLength << 5;
...
JSBI_gcl.__kBitsPerCharTableShift = 5;
JSBI_gcl.__kBitsPerCharTableMultiplier = 1 << JSBI_gcl.__kBitsPerCharTableShift;
JSBI_gcl.__kConversionChars = '0123456789abcdefghijklmnopqrstuvwxyz'.split('');
JSBI_gcl.__kBitConversionBuffer = new ArrayBuffer(8);
JSBI_gcl.__kBitConversionDouble = new Float64Array(JSBI_gcl.__kBitConversionBuffer);
JSBI_gcl.__kBitConversionInts = new Int32Array(JSBI_gcl.__kBitConversionBuffer);
...
JSBI_gcl.__clz30 = Math.clz32 ? function (x) {
